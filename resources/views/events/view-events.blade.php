        ...
        @foreach($events as $event)
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mb-4">
                <a href="{{route('events.show', $event->id)}}">
                    <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg flex  hover:bg-gray-200 dark:hover:bg-gray-700 ">
                        ...
                        <div class="flex flex-col p-9 leading-normallex justify-center">
                            <h5 class="mb-1 text-2xl font-bold tracking-tight text-gray-900 dark:text-white ">{{ $event['event_title'] }}</h5>
                            <p class="mb-2 font-normal text-xs text-gray-700 dark:text-gray-400">{{ $event['start_date'] }} @if($event['end_date']) - {{ $event['end_date'] }} @endif</p>
